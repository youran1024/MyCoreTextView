...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    HTView *htView = [[HTView alloc] initWithFrame:CGRectMake(0, 20, 320, 460)];
    [self.window addSubview:htView];
    [htView release];
    ...
    htView.string = fileString;
    htView.backgroundColor = [UIColor whiteColor];
...
{
    NSLog(@"end_coreNode:%@", node.string);
}

- (void)hTViewLinkClickedWhenTouchesBegin:(CoreNode *)node
{
    NSLog(@"coreNode:%@", node.string);
}
